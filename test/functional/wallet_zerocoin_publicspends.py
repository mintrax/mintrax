...
from test_framework.test_framework import MintraxTestFramework
...
class ZerocoinSpendTest(MintraxTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zbib_bal, bib_bal):
            zbib_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zbib_bal)
            bib_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], bib_bal)
            return zbib_bal, bib_bal
        ...
        zbib_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zbib_balance, 6666)
        ...
        zbib_balance, balance = check_balances(denom_2, zbib_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zbib_balance, balance = check_balances(denom_3, zbib_balance, balance)
        ...
        _, _ = check_balances(denom_1, zbib_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
