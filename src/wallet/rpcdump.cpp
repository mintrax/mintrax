...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"mintraxprivkey\" ( \"label\" rescan fStakingAddress )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"mintraxprivkey\"      (string, required) The private key (see dumpprivkey)\n"
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"mintraxaddress\"\n"
            "\nReveals the private key corresponding to 'mintraxaddress'.\n"
            "Then the importprivkey can be used with this output\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"mintraxaddress\"   (string, required) The mintrax address for the private key\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by MINT %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (pwalletMain->GetKey(seed_id, seed)) {
            ...
            CExtKey cointypeKey;           //key at m/purpose'/coin_type'  --> key at m/44'/840'
            CExtKey accountKey;            //key at m/purpose'/coin_type'/account' ---> key at m/44'/840'/0'
            CExtKey changeKey;             //key at m/purpose'/coin_type'/account'/change ---> key at m/44'/840'/0'/HDChain::ChangeType::ECOMMERCE.
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"mintraxaddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'mintraxaddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"mintraxaddress\"   (string, required) The mintrax address for the private key (you must hold the key already)\n"
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"mintraxaddress\" \"passphrase\"\n"
