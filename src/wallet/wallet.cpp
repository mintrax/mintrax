...
/**
 * Fees smaller than this (in uMINT) are considered zero fee (for transaction creation)
...
PairResult CWallet::getNewAddress(CTxDestination& ret, const std::string addressLabel, const std::string purpose, const CChainParams::Base58Type addrType)
{
    LOCK(cs_wallet);
                                        ...
                                        if (pcoin.IsTrusted() &&
                                            pcoin.GetDepthInMainChain() >=
                                                (Params().GetConsensus().NetworkUpgradeActive(chainActive.Height(), Consensus::UPGRADE_STAKE_MIN_DEPTH_V2) ?
                                                        Params().GetConsensus().nStakeMinDepthV2 :
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (nCoinType == STAKEABLE_COINS &&
                nDepth <
                    (Params().GetConsensus().NetworkUpgradeActive(chainActive.Height(), Consensus::UPGRADE_STAKE_MIN_DEPTH_V2) ?
                            Params().GetConsensus().nStakeMinDepthV2 :
                            Params().GetConsensus().nStakeMinDepth)) continue;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-MINT-address
...
std::string CWallet::CommitResult::ToString() const
{
    ...
    case CWallet::CommitStatus::NotAccepted:
        strErrRet += strprintf(_("WARNING: The transaction has been signed and recorded, so the wallet will try to re-send it. "
                                 "Use 'abandontransaction' to cancel it. (txid: %s)"),
            hashTx.ToString());
        break;
    default:
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in MINT) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of MINT"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart MINT to complete"));
            return nullptr;
        } else {
    ...
    if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
        if (prev_version <= FEATURE_PRE_MINT && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_MINT);
        }
