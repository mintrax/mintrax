...
#if defined(NDEBUG)
#error "MINT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uMINT) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("mintrax-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    unsigned int nMaxBlockSigOps = MAX_BLOCK_SIGOPS_CURRENT , MAX_BLOCK_SIGOPS_LEGACY;
    if (nSigOps > nMaxBlockSigOps)
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bibInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
           {
                bibInputs.push_back(stakeIn);
            }
        }
        const bool hasMINTInputs = !bibInputs.empty();
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasMINTInputs) {
                            for (const CTxIn& stakeIn : bibInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // MINT: We use certain sporks during IBD, so check to see if they are
