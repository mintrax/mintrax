...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MINT);
    ...
    if (settings.contains("nAnonymizeMintraxAmount") || reset)
        SoftSetArg("-anonymizemintraxamount", settings.value("nAnonymizeMintraxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mintrax.cpp during shotdown to also remove the window positions
