...
#include "config/mintrax-config.h"
#endif
...
#include "qt/mintrax/splash.h"
#include "qt/mintrax/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("mintrax-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in mintrax.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in mintrax.qrc)
...
/** Class encapsulating MINT Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (MINTGUI)
...
{
    window = new MINTGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MINTGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MINTGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(MINTGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MINTGUI::DEFAULT_WALLET);
        ...
        // MINT: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MINTGUI::handlePaymentRequest);
        connect(window, &MINTGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. MINT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(mintrax_locale);
    Q_INIT_RESOURCE(mintrax);
    ...
    /// 6. Determine availability of data directory and parse mintrax.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("MINT"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("MINT"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // mintrax: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("MINT didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
