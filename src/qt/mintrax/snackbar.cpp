...
#include "qt/mintrax/forms/ui_snackbar.h"
#include "qt/mintrax/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MINTGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MINTGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
