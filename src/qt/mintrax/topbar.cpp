...
#include "qt/mintrax/forms/ui_topbar.h"
#include "qt/mintrax/loadingdialog.h"
#include "qt/mintrax/lockunlock.h"
#include "qt/mintrax/qtutils.h"
#include "qt/mintrax/receivedialog.h"
...
TopBar::TopBar(MINTGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    ui->pushButtonConf->setButtonText("mintrax.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open mintrax.conf with default application"));
}
