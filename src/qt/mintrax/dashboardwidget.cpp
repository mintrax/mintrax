...
#include "qt/mintrax/forms/ui_dashboardwidget.h"
#include "qt/mintrax/sendconfirmdialog.h"
#include "qt/mintrax/txrow.h"
...
DashboardWidget::DashboardWidget(MINTGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MINT earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 MINT");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MINTGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &MINTGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &MINTGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MINT, zMINT
