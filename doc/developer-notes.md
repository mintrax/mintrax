...
- [Development guidelines](#development-guidelines)
    - [General MINT Core](#general-mintrax-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MINT Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MINTs (tMINT)" on the test network, if you
...
MINT Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mintrax910/mintrax/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mintrax
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mintrax --log_level=test_suite
$ valgrind -v --leak-check=full src/mintraxd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mintrax.coverage/index.html`.
```
...
- BitcoinMiner : Generates MINTs (if wallet is enabled).
...
However, in open source software such as MINT Core, where everyone uses
...
pay attention to for reviewers of MINT Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MINT Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MINT Core so that they can be integrated
...
file descriptors it will cause problems with MINT's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MINT-Project/MINT.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mintrax-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mintrax-cli` and the GUI debug console use this table to determine how to
