...
*Setup instructions for a gitian build of MINT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MINT
...
to the MINT GitHub release page.
...
- [Building MINT Core](#building-mintrax-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/mintrax-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/mintrax-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/mintrax-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/mintrax-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/mintrax-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/mintrax-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/mintrax-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@mintrax.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/mintrax-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/mintrax-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/mintrax-osx-*-build.assert
```
