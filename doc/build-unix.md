UNIX BUILD NOTES
====================
Some notes on how to build MINT Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile MINT Core and the dependencies,
...
This will build mintrax-qt as well, if the dependencies are met.
...
memory available when compiling MINT Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~mintrax/+archive/mintrax).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:mintrax/mintrax
    sudo apt-get update
...
To build MINT Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build mintrax-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a mintrax-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip mintraxd" to strip the debug
...
Security
--------
To help make your MINT Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, MINT Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, MINT Core may be compiled in
